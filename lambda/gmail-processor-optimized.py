...
class S3Uploader:
    ...
    def upload_attachment(self, user_email, sender_email, file_data, filename, message_id, mime_type):
        """Upload attachment to S3 with proper organization"""
        try:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"✓ Fetched {len(messages)} messages in {fetch_time:.2f}s")
        ...
        logger.info(f"   Files processed: {len(all_processed_files)}")
        ...
        if all_processed_files:
            try:
                ...
                batch_payload = {
                    "body": json.dumps({
                        "files": [file["s3_key"] for file in all_processed_files],
                ...
                if batch_processor_name:
                    ...
                    logger.info(f"📤 Batch processing triggered for {len(all_processed_files)} files")
                else:
        ...
        webhook_success = webhook_trigger.trigger_n8n_workflow(user_email, sender_email, all_processed_files)
