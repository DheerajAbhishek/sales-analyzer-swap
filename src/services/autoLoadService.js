...
class AutoLoadService {
    ...
    async getRestaurantNames(userRestaurants, restaurantIds) {
        ...
        if (userRestaurants?.restaurantMappings && userRestaurants.restaurantMappings.length > 0) {
            console.log('📋 Using restaurant mappings for names')
            restaurantIds.forEach(id => {
                const mapping = userRestaurants.restaurantMappings.find(r => r.id === id)
                if (mapping && mapping.name && !mapping.name.startsWith('Restaurant ')) {
            ...
            try {
                ...
                const metadataPromises = restaurantIds.map(async (restaurantId) => {
                    try {
                        const metadata = await restaurantService.getRestaurantMetadata(restaurantId)
                        return {
                            id: restaurantId,
                        ...
                        console.warn(`⚠️ Failed to fetch metadata for ${restaurantId}:`, error.message)
                        return {
                            id: restaurantId,
    ...
    async loadLastMonthData(userRestaurants) {
        try {
            console.log('🚀 Auto-loading last month data for user restaurants:', userRestaurants)
            ...
            console.log(`🔍 Will try ${allRestaurantIds.length} restaurants until data is found...`)

            for (let i = 0; i < allRestaurantIds.length; i++) {
                const restaurantId = allRestaurantIds[i]
                console.log(`� Trying restaurant ${i + 1}/${allRestaurantIds.length}: ${restaurantId}`)

                try {
                    // Get restaurant name for this specific restaurant
                    const restaurantData = await this.getRestaurantNames(userRestaurants, [restaurantId])
                    const restaurant = restaurantData[0]

                    console.log(`📈 Fetching data for restaurant: ${restaurant.name} (${restaurantId})`)
                    ...
                    if (parsed.consolidatedInsights && Object.keys(parsed.consolidatedInsights).length > 0) {
                        const insights = parsed.consolidatedInsights
                        if (insights.noOfOrders > 0 || insights.grossSale > 0) {
                            console.log(`✅ Found data for restaurant: ${restaurant.name} (${restaurantId})`)
                    ...
                    console.log(`📭 No meaningful data found for restaurant: ${restaurant.name} (${restaurantId})`)

                } catch (error) {
                    console.warn(`⚠️ Failed to fetch data for restaurant ${restaurantId}:`, error.message)
