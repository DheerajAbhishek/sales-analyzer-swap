...
def _calculate_final_metrics(data):
    ...
    packings = data.get('packings', 0.0)
    
    # Use already-summed values from daily reports
    net_order = data.get('netOrder', 0.0)
    total_deductions = data.get('totalDeductions', 0.0)
    net_pay = data.get('netPay', 0.0)
    ...
    # Gross Sale With GST
    gross_sale_with_gst = data.get('grossSaleWithGST', gross_sale + gst_on_order)

    return {
        "noOfOrders": data.get('processedOrdersCount', 0),
        "grossSale": round(gross_sale, 2),
        "grossSaleWithGST": round(gross_sale_with_gst, 2),
        "grossSaleAfterGST": round(data.get('grossSaleAfterGST', gross_sale), 2),
        "gstOnOrder": round(gst_on_order, 2),
        "discounts": round(discounts, 2),
        "packings": round(packings, 2),
        "ads": round(ads, 2),
        ...
        # Use already-summed metrics from daily reports
        "netOrder": round(net_order, 2),
        "totalDeductions": round(total_deductions, 2),
        "netPay": round(net_pay, 2),
        # Breakdowns will be merged separately - placeholders here
        "netOrderBreakdown": {},
        "deductionsBreakdown": {},
        # Zomato-specific metrics
        "deliveredOrdersCount": data.get('deliveredOrdersCount', 0),
        "cancelledOrdersCount": data.get('cancelledOrdersCount', 0),
        "rejectedOrdersCount": data.get('rejectedOrdersCount', 0),
        "cancelledPayout": round(data.get('cancelledPayout', 0.0), 2),
        "rejectedPayout": round(data.get('rejectedPayout', 0.0), 2)
    }
...
def merge_breakdowns(reports, breakdown_key):
    """Merge nested breakdown structures from multiple reports."""
    def to_float(val):
    ...
    if breakdown_key == "netOrderBreakdown":
        consolidated = {
            "subtotal": 0.0,
            "packaging": 0.0,
            "discountsPromo": 0.0,
        ...
        for report in reports:
            ...
            for key in consolidated.keys():
                consolidated[key] += to_float(breakdown.get(key, 0.0))
        return {k: round(v, 2) for k, v in consolidated.items()}
    
    elif breakdown_key == "deductionsBreakdown":
        consolidated = {
            "commission": {
                "baseServiceFee": 0.0,
                "paymentMechanismFee": 0.0,
                "longDistanceFee": 0.0,
                "serviceFeeDiscount": 0.0,
                "pocketHeroFees": 0.0,
                "boltFees": 0.0,
                "deliveryFeeSponsored": 0.0,
                "callCenterCharges": 0.0,
                "swiggyOneFees": 0.0,
                "total": 0.0
            },
            "taxes": {
        ...
        for report in reports:
            ...
            # Sum commission components
            commission = breakdown.get("commission", {})
            for key in consolidated["commission"].keys():
                consolidated["commission"][key] += to_float(commission.get(key, 0.0))
            
            # Sum tax components
            taxes = breakdown.get("taxes", {})
            for key in consolidated["taxes"].keys():
                consolidated["taxes"][key] += to_float(taxes.get(key, 0.0))
            
            # Sum other deductions
            consolidated["otherDeductions"] += to_float(breakdown.get("otherDeductions", 0.0))
            consolidated["totalDeductions"] += to_float(breakdown.get("totalDeductions", 0.0))
        
        # Round all values
        result = {
            "commission": {k: round(v, 2) for k, v in consolidated["commission"].items()},
            "taxes": {k: round(v, 2) for k, v in consolidated["taxes"].items()},
            "otherDeductions": round(consolidated["otherDeductions"], 2),
            "totalDeductions": round(consolidated["totalDeductions"], 2)
        }
        return result
    ...
    try:
        ...
        if not insight_files:
            return { 
                ...
                "body": json.dumps({"message": "No data found for this restaurant."}) 
            }
        ...
        if not reports_in_range:
            return { 
                ...
                "body": json.dumps({"message": "No data found for the selected date range."}) 
            }

        sum_keys = [
            "processedOrdersCount", "grossSale", "grossSaleWithGST", "grossSaleAfterGST", "gstOnOrder",
            "discounts", "packings", "commissionAndTaxes",
            "payout", "ads", "netSale", "deliveredOrdersCount",
            "cancelledOrdersCount", "rejectedOrdersCount", "cancelledPayout",
            "rejectedPayout", "netOrder", "totalDeductions", "netAdditions", "netPay"
        ]
        
        # --- NEW GROUPING LOGIC ---
        if group_by in ['day', 'month', 'week']:
            grouped_data = {}
            for report in reports_in_range:
                ...
                if group_by == 'day':
                    period_key = report_date.strftime('%Y-%m-%d') # e.g., "2025-08-01"
                elif group_by == 'month':
                ...
                if platform not in grouped_data[period_key]:
                    grouped_data[period_key][platform] = {
                        'summed_keys': {key: 0 for key in sum_keys},
                        'reports': []
                    }
                
                for key in sum_keys:
                    grouped_data[period_key][platform]['summed_keys'][key] += report.get(key, 0)
                grouped_data[period_key][platform]['reports'].append(report)
            
            time_series_data = []
            for period, platforms in sorted(grouped_data.items()):
                period_data = {"period": period}
                for platform, platform_data in platforms.items():
                    metrics = _calculate_final_metrics(platform_data['summed_keys'])
                    
                    # Merge breakdowns for this period and platform
                    net_order_breakdown = merge_breakdowns(platform_data['reports'], "netOrderBreakdown")
                    deductions_breakdown = merge_breakdowns(platform_data['reports'], "deductionsBreakdown")
                    
                    metrics["netOrderBreakdown"] = net_order_breakdown
                    metrics["deductionsBreakdown"] = deductions_breakdown
                    
                    period_data[platform] = metrics
        ...
        # --- ORIGINAL CONSOLIDATION LOGIC (if not grouping) ---
            ...
            # Merge the breakdowns from daily reports
            net_order_breakdown = merge_breakdowns(reports_in_range, "netOrderBreakdown")
            deductions_breakdown = merge_breakdowns(reports_in_range, "deductionsBreakdown")
            
            # Override the calculated breakdowns with the merged ones
            final_insights["netOrderBreakdown"] = net_order_breakdown
            final_insights["deductionsBreakdown"] = deductions_breakdown
            
            discount_breakdown = merge_discount_breakdowns(reports_in_range)
            
            final_response_body = {
                "consolidatedInsights": final_insights, 
