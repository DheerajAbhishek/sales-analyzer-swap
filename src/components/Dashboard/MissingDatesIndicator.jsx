...
const MissingDatesIndicator = ({ timeSeriesData, selections, dataType, user }) => {
    ...
    useEffect(() => {
        ...
        } else if (dataType === 'total' && selections?.startDate && selections?.endDate && selections?.restaurants) {
            // For total summary, check missing dates for each restaurant and platform
            console.log('📊 Total summary mode - checking missing dates via API for multiple platforms')
            setLoading(true)
            ...
            const checkMissingDatesForAllPlatforms = async () => {
                ...
                try {
                    ...
                    for (const restaurantId of selections.restaurants) {
                        console.log(`🔍 Checking missing dates for restaurant: ${restaurantId}`)
                        ...
                        if (restaurant) {
                            ...
                            console.log(`Restaurant group ${restaurant.name} has filtered platform IDs:`, platformIds)
                        } else {
                            ...
                            if (selections.channels.includes(guessedChannel)) {
                                platformIds = [{ channel: guessedChannel, platformId: restaurantId }]
                            } else {
                        ...
                        for (const platform of platformIds) {
                            ...
                            const channel = platform.channel || userRestaurantMappingService.guessChannelForId(platformId)

                            try {
                                console.log(`📅 Checking missing dates for platform ${platformId} (${channel})`)
                                ...
                                if (result.success) {
                                    const key = restaurant ? `${restaurant.name} (${channel})` : `${platformId} (${channel})`
                                    results[key] = {
                                    ...
                                    console.log(`✅ Missing dates for ${key}:`, result.data.missingDates?.length || 0)
                                } else {
                                    console.log(`❌ Failed to get missing dates for ${platformId}:`, result.error)
                                }
                            } catch (error) {
                                console.error(`❌ Error checking missing dates for ${platformId}:`, error)
                            }
                        }
                    }
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div>
                            ...
                            <p style={{ margin: 0, fontSize: '0.8rem', color: '#0277bd' }}>
                                All {daysDiff + 1} day{daysDiff !== 0 ? 's' : ''} have data available across {platformKeys.length} platform{platformKeys.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                    </div>
                </div>
    ...
    const hasPlatformSpecificData = dataType === 'total' && Object.keys(missingDatesData).length > 1
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <div>
                    ...
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#a16207' }}>
                        {calculatedMissingDates.length} {calculatedMissingDates.length === 1 ? 'date' : 'dates'} with no data found
                        {hasPlatformSpecificData && ` across ${Object.keys(missingDatesData).length} platforms`}
                    </p>
                </div>
                <button
            ...
            {showDetails && (
                <div style={{
                    ...
                    {hasPlatformSpecificData ? (
                        ...
                        <div>
                            <p style={{ margin: '0 0 8px 0', fontWeight: '600' }}>Missing dates by platform:</p>
                            {Object.entries(missingDatesData).map(([platformKey, data]) => {
                                if (!data.missingDates || data.missingDates.length === 0) return null
                                ...
                                return (
                                    <div key={platformKey} style={{ marginBottom: '4px' }}>
                                        <strong>{platformKey}:</strong> {platformFormatted.join(', ')}
                                        <span style={{ opacity: 0.7 }}>
                                            ({data.missingDates.length} {data.missingDates.length === 1 ? 'date' : 'dates'})
                                        </span>
                                    </div>
                                )
                            })}
