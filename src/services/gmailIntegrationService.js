...
const API_BASE_URL = 'https://p28ja8leg9.execute-api.ap-south-1.amazonaws.com/Production'
...
class GmailIntegrationService {
    ...
    async checkGmailTokens(userEmail) {
        try {
            ...
            console.log('🧪 TESTING: Using POST method to check tokens (temporary workaround)')
    ...
    async processEmailsFromSender(userEmail, senderEmail, maxResults = 150) {
        try {
            console.log(`🔄 Processing Excel files from ${senderEmail} for user: ${userEmail}`)
    ...
    async removeGmailTokens(userEmail) {
        try {
            ...
            const response = await fetch(`${this.gmail_tokens_url}/${encodeURIComponent(userEmail)}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                }
    ...
    async handleGmailAuthCallback(code, state, userEmail) {
        try {
            ...
            const integrationResult = await this.initializeGmailIntegration(userEmail)
