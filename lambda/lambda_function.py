...
def lambda_handler(event, context):
    ...
    try:
        ...
        daily_results = []
        max_workers = min(10, len(dates))  # Limit concurrent requests to avoid rate limiting
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all fetch tasks
            future_to_day = {
                executor.submit(fetch_sales_for_day, day, branch_id, api_key, secret_key, api_url): day 
                for day in dates
            }
            ...
            for future in as_completed(future_to_day):
                ...
                try:
                    ...
                    daily_results.append([])  # Append empty list for failed days
