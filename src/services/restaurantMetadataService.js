...
export const restaurantMetadataService = {
  ...
  async getRestaurantMetadata() {
    try {
      ...
      const backendResult =
        await restaurantMappingService.getRestaurantMetadata();
      ...
      console.warn(
        "Failed to fetch metadata from backend, using localStorage:",
        error,
      );
    }
  ...
  async saveRestaurantData(restaurantId, data) {
    try {
      ...
      try {
        ...
        const backendResult =
          await restaurantMappingService.saveRestaurantMetadata(allMetadata);
      ...
      window.dispatchEvent(
        new CustomEvent("restaurantMetadataUpdated", {
          detail: {
            restaurantId,
  ...
  async initializeRestaurantData(restaurantIds) {
    ...
    if (hasChanges) {
      ...
      window.dispatchEvent(
        new CustomEvent("restaurantMetadataInitialized", {
