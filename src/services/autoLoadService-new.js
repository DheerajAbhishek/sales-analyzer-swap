...
class AutoLoadService {
    ...
    async loadLastMonthData(userRestaurants) {
        try {
            console.log('🚀 Auto-loading recent data for user restaurants:', userRestaurants)
            ...
            const result = await dateService.getLastAvailableDate(firstRestaurantId, businessEmail)
            ...
            const fetchPromises = restaurantIds.map(async (restaurantId) => {
                try {
                    ...
                    console.warn(`⚠️ Failed to fetch data for ${restaurantId}:`, error.message)
                    return {
                        success: false,
                        error: error.message,
                        detail: {
            ...
            console.log(`✅ Auto-loaded data for ${successfulResults.length} restaurants`)
            ...
            return {
                ...
                excludedChannels: results.filter(result => !result.success).map(result => ({
