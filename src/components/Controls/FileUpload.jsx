...
const FileUpload = () => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (statusData.status === "IN_PROGRESS") {
          setUploadStatus(
            `⏳ Processing file ${statusData.processedCount} of ${statusData.totalFiles}...`,
          );
        } else if (statusData.status === "COMPLETED") {
          setUploadStatus(
            `✅ All ${statusData.totalFiles} files processed successfully!`,
          );
          clearInterval(interval);
        } else if (statusData.status === "FAILED") {
          setUploadStatus(
            `🔥 ERROR: Processing failed. Check logs for Job ID ${jobId}.`,
          );
          clearInterval(interval);
        }
      } catch (err) {
  ...
  return (
    <div className="card">
      ...
      {uploadStatus && <div className={getStatusClass()}>{uploadStatus}</div>}
