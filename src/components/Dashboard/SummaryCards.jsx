...
const SummaryCards = ({
  ...
  const [discountModalOpen, setDiscountModalOpen] = useState({
    isOpen: false,
    position: null,
  });
  ...
  const cardData = [
    ...
    },
    {
      ...
      percentage:
        data.adsPercent ||
        calculatePercentage(data.ads, data.grossSaleAfterGST),
  ...
  const handleDiscountBreakdown = (e) => {
    ...
    if (!discountBreakdown) {
      alert(
        "Discount breakdown is only available for Swiggy and Zomato channels",
      );
      return;
    }
                  ...
                  style={{
                    ...
                    color:
                      item.threshold !== undefined
                        ? getThresholdColor(item.percentage, item.threshold)
                        : "#6b7280",
                    margin: "4px 0 0 0",
                    fontWeight:
