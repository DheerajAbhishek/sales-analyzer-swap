...
const OAuthCallback = ({ onAuthSuccess }) => {
    ...
    useEffect(() => {
        const handleCallback = async () => {
            ...
            if (processingRef.current || (code && processedCodeRef.current === code)) {
                console.log('⏳ OAuth callback already processed for this code, skipping...')
                return
            }

            processingRef.current = true
            if (code) processedCodeRef.current = code
            console.log('🚀 Starting OAuth callback processing for code:', code?.substring(0, 10) + '...')
