...
class GoogleApisService {
    ...
    async getSalesRelatedEmails(options = {}) {
        const {
            ...
            keywords = ['order', 'purchase', 'invoice', 'receipt', 'sale', 'customer']
        } = options
        ...
        try {
            ...
            const messages = await googleOAuthService.getGmailMessages(query, maxResults)
    ...
    categorizeEmail(messageDetails) {
        ...
        if (text.includes('order') || text.includes('purchase') || text.includes('bought')) {
            return 'orders'
        } else if (text.includes('receipt') || text.includes('invoice') || text.includes('payment')) {
            return 'receipts'
        } else if (text.includes('question') || text.includes('inquiry') || text.includes('help')) {
            return 'inquiries'
        } else if (text.includes('review') || text.includes('feedback') || text.includes('rating')) {
            return 'feedback'
        } else {
            return 'other'
        }
    ...
    async getSalesRelatedFiles(options = {}) {
        ...
        try {
            ...
            const keywordQueries = keywords.map(keyword => `name contains '${keyword}'`)
    ...
    categorizeFile(file) {
        ...
        if (mimeType.includes('spreadsheet')) {
            return 'spreadsheets'
        } else if (name.includes('report') || name.includes('analytics') || name.includes('summary')) {
            return 'reports'
    ...
    async exportToGoogleSheets(data, sheetName = 'Sales Data Export') {
        try {
            ...
            const spreadsheet = await googleOAuthService.makeAuthenticatedRequest(createUrl, {
                method: 'POST',
