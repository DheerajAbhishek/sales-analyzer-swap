...
const ReportControls = ({ onGetReport, loading, userRestaurants }) => {
            ...
            try {
              ...
              if (restaurantMappings && restaurantMappings.length > 0) {
                restaurantMappings.forEach((restaurant) => {
                  ...
                  Object.values(restaurant.platforms || {}).forEach(
                    (platformId) => {
                      if (platformId) assignedPlatformIds.add(platformId);
                    },
                  );
              ...
              userRestaurants.restaurantIds.forEach((restaurantId) => {
                if (!assignedPlatformIds.has(restaurantId)) {
                  options.push({
                    ...
                    label: restaurantId,
                    platforms: {
                      [userRestaurantMappingService.guessChannelForId(
                        restaurantId,
                      )]: restaurantId,
                    },
                    isGroup: false,
  ...
  const fetchLastAvailableDate = async (restaurantId) => {
    ...
    try {
      ...
      if (successfulResults.length > 0) {
        ...
        const totalDatesAcrossAll = successfulResults.reduce(
          (sum, r) => sum + (r.totalDates || 0),
          0,
        );
        ...
        const failedMessages = results
          .map((r) => `${r.platformId}: ${r.message || "No data"}`)
          .join("; ");
        setRestaurantLastDates((prev) => ({
          ...prev,
          [restaurantId]: {
  ...
  const removeRestaurant = (restaurantId) => {
    const newSelectedRestaurants = selectedRestaurants.filter(
      (id) => id !== restaurantId,
    );
  ...
  const saveThresholdSetting = async (type, value) => {
    try {
      ...
      if (!currentUser) {
        const errorMsg = "No authenticated user found. Please log in to save threshold settings.";
      ...
      if (!userId) {
        const errorMsg = "User ID not available. Cannot save threshold settings.";
        console.error(errorMsg);
        setThresholdError(errorMsg);
        return;
      }
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      alert(
        "Please select at least one restaurant, one channel, and a date range.",
      );
      return;
    }
  ...
  const findRestaurantAndChannel = (platformId) => {
    const restaurantInfo =
      userRestaurantMappingService.findRestaurantByPlatformId(platformId);
    ...
    <div className="card">
      ...
      <div className="form-group">
        ...
        {!userRestaurants ? (
          ...
          >
            No restaurants found in your account. Please upload some data files
            first.
          </div>
        ) : (
          <Select
                        ...
                        {lastDateInfo.activePlatforms > 1
                          ...
                          lastDateInfo.platformResults
                            ...
                            .map((result, index) => {
                              ...
                              if (restaurant) {
                                const platformChannel = Object.entries(
                                  restaurant.platforms || {},
                                ).find(
                                  ([channel, id]) => id === result.platformId,
                                )?.[0];
                                if (platformChannel) {
                                  platformName =
                                    platformChannel.charAt(0).toUpperCase() +
                                    platformChannel.slice(1);
                                }
                              }

                              return (
                                <div key={result.platformId}>
                                  {platformName}: {result.date} (
                                  {result.totalDates} days)
                                </div>
                              );
                            })
      ...
      <div className="form-group">
        <h4 className="form-label">
          {selectedChannels.length === 1 ? "5" : "4"}. Group Data By
        </h4>
