...
This function lists objects in an S3 bucket under the prefix `users/{encoded_email}/` and
extracts candidate restaurant IDs from the object keys. It returns a JSON array of
...
def _extract_restaurant_id_from_key(key, prefix):
    """
    Given a full S3 key and the prefix (e.g. users/admin_at_swapnow_dot_in/), return a
    candidate restaurant id. 
    
    Expected structure: users/{encoded_email}/daily-insights/{restaurantId}/file.json
    So from: users/admin_at_swapnow_dot_in/daily-insights/19251816/2025-09-01.json
    Extract: 19251816
    """
    ...
    # Split the path: daily-insights/19251816/2025-09-01.json
    segments = remainder.split('/')
    
    # If we have at least 2 segments and first is 'daily-insights'
    if len(segments) >= 2 and segments[0] == 'daily-insights':
        restaurant_id = segments[1]
        return restaurant_id
    
    # Fallback: take first segment if no daily-insights pattern
    first = segments[0]
    if '.' in first:
