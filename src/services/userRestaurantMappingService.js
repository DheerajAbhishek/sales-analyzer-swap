...
export const userRestaurantMappingService = {
    ...
    async getUserRestaurantMappings() {
        try {
            ...
            const backendResult = await restaurantMappingService.getRestaurantMappings()

            if (backendResult.success && backendResult.data) {
                // Save to localStorage as backup
                localStorage.setItem('userRestaurantMappings', JSON.stringify(backendResult.data))
                return backendResult.data
            }
        } catch (error) {
            console.warn('Failed to fetch mappings from backend, using localStorage:', error)
        }
    ...
    async saveUserRestaurantMappings(mappings) {
        ...
        try {
            ...
            const backendResult = await restaurantMappingService.saveRestaurantMappings(mappings)
        ...
        try {
            ...
            window.dispatchEvent(new CustomEvent('userRestaurantMappingsUpdated', {
    ...
    async findRestaurantByPlatformId(platformId) {
        ...
        for (const restaurant of mappings) {
            for (const [channel, mappedPlatformId] of Object.entries(restaurant.platforms)) {
    ...
    async getRestaurantPlatformIds(restaurantId) {
        ...
        return Object.values(restaurant.platforms).filter(id => id && id.trim() !== '')
    },
